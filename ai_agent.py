...
class SQLAgent:
    ...
    def generate_er_diagram(self, table_names=None, output_file="er_diagram.png", output_format="png", log=False):
        if not self.metadata.tables:
            print("Нет данных для создания диаграммы. Загрузите базу данных.")
            return

        # Фильтрация таблиц на основе запроса пользователя
        if table_names:
            # Отфильтровать таблицы, которые есть в базе данных
            selected_tables = {name: table for name, table in self.metadata.tables.items() if name in table_names}
            if not selected_tables:
                print(f"Не удалось найти таблицы: {', '.join(table_names)} в базе данных.")
                return
        else:
            # Если не указаны, использовать все таблицы
            selected_tables = self.metadata.tables
        ...
        try:
            ...
            # Создаем граф ER-диаграммы с учётом выбранных таблиц
            graph = create_schema_graph(
                ...
                rankdir='LR',
                concentrate=True,
                tables=list(selected_tables.values())  # Указываем только выбранные таблицы
            )
    ...
    def infer_relationships(self):
        """Логически создает связи между таблицами на основе имен колонок и типов данных."""
        relationships = []

        for table_name, info in self.tables_info.items():
            for column_name, column_type in info['columns'].items():
                # Проверяем, является ли колонка первичным ключом
                if column_name in info['primary_keys']:
                    continue

                # Поиск потенциальных связей с другими таблицами
                for other_table_name, other_info in self.tables_info.items():
                    if table_name == other_table_name:
                        continue

                    for other_column_name, other_column_type in other_info['columns'].items():
                        # Проверка на схожесть имен и совместимость типов
                        if (column_name.endswith("_id") and other_column_name == column_name) or \
                                (other_column_name.endswith("_id") and column_name == other_column_name):
                            # Убедимся, что типы совместимы
                            if str(column_type) == str(other_column_type):
                                relationships.append((table_name, column_name, other_table_name, other_column_name))
                                print(
                                    f"Предложена связь: {table_name}.{column_name} -> {other_table_name}.{other_column_name}")

        # Если были найдены связи, можно их добавить в базу данных как внешние ключи
        for relationship in relationships:
            table, column, other_table, other_column = relationship
            alter_query = f"ALTER TABLE {table} ADD CONSTRAINT fk_{table}_{column}_{other_table}_{other_column} FOREIGN KEY ({column}) REFERENCES {other_table}({other_column});"
            print(f"Выполняется запрос для создания внешнего ключа: {alter_query}")
            # Здесь можно добавить код для выполнения SQL-запроса, если это необходимо
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Логическая генерация связей
        agent.infer_relationships()
    ...
    while True:
        try:
            ...
            if 'диаграмм' in user_query:
                if 'все' in user_query:
                    # Генерация ER-диаграммы для всех таблиц
                    agent.generate_er_diagram()  # Вызов без параметров для всех таблиц
                else:
                    # Получение списка таблиц для ER-диаграммы от пользователя
                    user_input = input(
                        "Введите имена таблиц через запятую для создания ER-диаграммы (например, table1,table2): ").strip()
                    table_names = [name.strip() for name in user_input.split(',') if name.strip()]

                    # Вызов метода для создания диаграммы с выбранными таблицами
                    if table_names:
                        agent.generate_er_diagram(table_names=table_names)
                    else:
                        print("Не указаны таблицы для генерации ER-диаграммы.")
            else:
